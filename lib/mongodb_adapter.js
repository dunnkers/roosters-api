...
exports.setScheduleRelations = function (collection) {
	return exports.findAll(collection).then(function (docs) {
		...
		function recurse (list) {
			var docs = list.splice(0, 10);
			return exports.resolveSchedules(collection, docs).then(function (docs) {
				return RSVP.all(docs.map(function (doc) {
					return exports.update(collection, doc._id, {
						$set: { timetableRelations: _.first(doc.timetable) }
					});
				}));
			}).then(function (res) {
				print(format('RESOLVED %d\t\t', docs.length));
				results = results.concat(res);
				return list.length ? recurse(list) : results;
			});
		}
		return recurse(docs);
	});
...
exports.resolveSchedules = function (collection, docs) {
	return RSVP.all(docs.map(function (doc) {
		return exports.resolveSchedule(collection, doc);
	}));
};*/

exports.resolveSchedule = function (collection, doc) {
	return RSVP.all(_.flatten(_.first(doc.timetable).map(function (day, i) {
		return day.map(function (hour, j) {
			...
			return exports.find(collection, query).then(function (docs) {
				...
				_.first(doc.timetable)[i][j] = students && students.length ? {students: students} : {};
			});
		});
	}))).then(function () {
		return doc;
